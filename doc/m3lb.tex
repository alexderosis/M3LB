...
\noindent Low by $1.9\times$ to $4.3\times$, \emph{at $t=0$}, before any
dynamics. Two explanations were tested and both fail: it is not the spectrum cut
(sweeping \code{-kmax} over $24,40,60,80,120$ gives $7.6,8.9,7.3,4.8,9.8$,
non-monotonic because the central-difference curl under-resolves $b$ near the
Nyquist) and it is not the gauge (fixing zero mean moves it by $0.1\,\%$). This
is the sharpest single discrepancy in the comparison and it is \textbf{open}.
