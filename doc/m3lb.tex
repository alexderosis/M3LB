...
rejected at setup rather than approximated.
...
scalar wall, which does not exist here (\S\ref{sec:known}).
...
number where the reference's $H=499$ gives $1.6$ and $4.2$. Refining settles it. At
$T=5000$, $N_e$ is $2.8558$ at $N=81$, $2.8017$ at $N=129$ and $2.4903$ at $N=201$
($\mathrm{Re}_{\rm cell}=8.5$, $6.1$, $3.6$); unseeded at $N=201$ it is $2.4366$. The
sequence falls monotonically, so the coarse grid was \emph{passing through} the published
value rather than reproducing it, and the converged deficit is the same $\sim\!13\,\%$ the
well-resolved mid-range shows. Halving $u_0$ at $N=129$ moves $2.8017$ to $2.7104$, so
compressibility is a $3\,\%$ term and not the explanation. When the badly-behaved runs
agree and the well-behaved ones do not, the agreement is what to distrust.
...
\item \textbf{The closed cavity's electric Nusselt number is $9$--$13\,\%$ below
  the reference, and that is measured rather than explained}
  ...
  the level is low wherever the grid is trustworthy. Four candidates are
  excluded: refinement moves the answer \emph{away} from the reference rather
  than toward it ($2.8558\to2.8017\to2.4903$ at $N=81,129,201$), halving $u_0$
  moves it $3\,\%$, the seed is excluded by the unseeded sweep, and the wall
  columns contribute under $1\,\%$. No fifth candidate has been tested. The
  reference's $500^2$ grid is what the cell Reynolds number says $T\ge5000$
  needs and costs about five hours per $T$ on one machine, so the comparison at
  matched resolution has not been made.

\item \textbf{The two specular walls sit on different planes.} The fluid's
  (\S\ref{sec:specular}) is a ghost cell, so its mirror is half a cell outside
  the last fluid node; the scalar's (\S\ref{sec:scalarspec}) is on-node, because
  the Dirichlet family it must pair with is on-node. Each is exact against the
  box it mirrors --- $5.5\times10^{-14}$ and $5.3\times10^{-15}$ --- but they are
  exact about planes half a cell apart. They pair in \S\ref{sec:ehdcavity},
  where every wall is on-node; they do \emph{not} pair in
  \S\ref{sec:ehdconv}'s free-slip variant, so that geometry's gap must not be
  read as a measurement of the scalar wall alone. Closing it needs an on-node
  specular fluid wall, which does not exist here. Axis-aligned normals only, and
  no specular corners, in either.
