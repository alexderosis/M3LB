...
\subsection{The Reynolds definition, and a caution on Table~I}

Eq.~(7) writes $E$ as an integral over $V_f$, but $\mathrm{Re} = 2r\sqrt{2E_u}/\nu$ is a
Reynolds number only if $\sqrt{2E_u}$ is a \emph{velocity}: in two dimensions an
integral of $|u|^2$ over an area has units $L^4/T^2$, whose square root is
$L^2/T$, and $2r(L^2/T)/\nu$ then has units of length. The two cannot both be
literal. The mean reading --- $E_u$ per unit area --- gives
$u_{\mathrm{rms}}\approx 0.65$--$1.33$ and reproduces the quoted $\mathrm{Re}$ by
construction; the integral reading gives $u_{\mathrm{rms}}\approx0.12$--$0.25$ and
a physical Reynolds number of only $731$--$1497$, far too low for the $k^{-3}$
turbulence of their Figs.~7--8. So $E_u$ is per unit area, this tree's $\mathrm{Re}$
means the same thing, and the runs really are a factor $3.9$ below the reference
rather than above it.

Table~I's $\mathrm{Re}$ column is nonetheless \emph{not} derivable from its other
columns. Eq.~(10) normalises to $E_B = 1/2$, so $E_u/E_B$ alone would fix $E_u$
and hence $\mathrm{Re}$; the implied $E_B$ instead comes out $0.70$, $4.6\times10^{-5}$,
$0.29$, $0.46$, with regime~II off by four orders of magnitude. Each regime is its
own realisation and the absolute amplitude was not held fixed across the four.
That is the second place Table~I does not close against the paper's own
equations.
...
Dynamic alignment is not sustained. Both curves rise to the same peak and then
part: ours reaches $\cos\theta = 0.880$ at $t=27$ against the reference's $0.864$
at $t=30$, agreeing to $1.9\,\%$ in amplitude and about $10\,\%$ in timing, after
which ours decays to $0.029$ where theirs climbs to $0.847$. The corroborating
diagnostic is $E/|H_c|$, whose value $2$ is the paper's own signature of
alignment: ours leaves it, $3.45 \to 9.89 \to 48.7 \to 119$, where theirs holds
$2.04$ throughout. So the mechanism is captured and not held --- a sharper
statement than ``alignment not reproduced'', and one that only became available
when regime~III became runnable.

The intermediate-time decay exponents are two to three times too steep, and that
is structural rather than a defect: fifty crossings is $t=450$ on the
\emph{advective} clock, but at $\mathrm{Re}=1000$ against their $3868$--$7920$ it is
$\nu t/R^2 \approx 0.20$ against their $0.0505$, four times further into
diffusive decay. Both clocks cannot be matched at mismatched Reynolds number.

Regime~II's $E_u/E_B$ disagrees qualitatively --- theirs rises four decades,
ours falls --- but $E_{\mathrm{mag}}$ there is $2\times10^{-11}$, four decades
below the seeded field, so the late ratio is plausibly a numerical floor. It is
reported as uninterpretable rather than as a discrepancy.
...
control. The $E/A$ shortfall is no longer among the open items: it is a property
of the reference, as \S\ref{sec:neffaa} shows.
...
\item \textbf{The EHD stack is not in \code{GPU/}.} It is the newest module and
  the CUDA implementation has not been extended to it, so the fine grids that
  \S\ref{sec:ehdconv} wants --- $196\times320$ and beyond --- are hours of host
  time rather than minutes of device time.
