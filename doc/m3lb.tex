...
\subsection{Density-anomaly buoyancy: water near \texorpdfstring{4\,$^\circ$C}{4 C}}
\label{sec:density-anomaly}

Water is densest at $T_m \approx 4\,^\circ$C, and Gebhart \& Mollendorf (1977)
fit pure water at one atmosphere by
$\rho(T) = \rho_m\big(1 - w_{GM}\,|T - T_m|^{q}\big)$ with
$\rho_m = 999.972$\,kg\,m$^{-3}$, $w_{GM} = 9.297173\e{-6}\,(^\circ\mathrm{C})^{-q}$,
$q = 1.894816$ and $T_m = 4.029325\,^\circ$C. \code{DensityAnomalyGuo} delivers
the corresponding buoyancy through the Guo machinery,
\begin{equation}
  \bm{F}(n) = \rho_0\, \hat{\bm{e}}_{\uparrow}\, w\,
  \big(|T(n)-T_m|^{q} - |T_0-T_m|^{q}\big),
\end{equation}
with $w = g\,w_{GM}\,\Delta T^{q}$ in lattice units when the scalar carries
$\theta = (T - T_{\rm ref})/\Delta T$. Over 0--8\,$^\circ$C the density moves by
$1.2\e{-4}$ of itself, so the approximation is Boussinesq exactly as in
Sec.~\ref{sec:boussinesq}; only the shape of the buoyancy changes, and it is not
monotone: water between $0$ and $4\,^\circ$C heated from below is unstable, and
above $4\,^\circ$C stable. For $q=1$ with every temperature above $T_m$ the
policy is \code{BoussinesqGuo} with $\beta = w$, and the two agree on a growing
Rayleigh--B\'enard mode to $1.7\e{-15}$ in $T$ and $7.5\e{-14}$ in $v$ (not bit
for bit, because $|T-T_m|-|T_0-T_m|$ and $T-T_0$ round differently).

\textbf{Validation.} \code{validation/density\_anomaly} brackets the onset of
penetrative convection in a rigid--rigid layer with $b(\theta) =
|\theta-\lambda|^{q}$ against an exact reference: \code{tools/penetrative\_onset.py}
solves the marginal equations by shooting, on a mesh split and graded at the
kink $z = 1-\lambda$, and reproduces the classical $\mathrm{Ra}_c = 1707.762$,
$k_c H = 3.117$ before it reports anything. At the Gebhart--Mollendorf $q$ the
references are $\mathrm{Ra} = 16764.642$ at $kH = 4.0212$ for $\lambda = 0.5$
(maximum mid-layer, upper half stably stratified; the minimum is
$\mathrm{Ra}_c = 16763.68$ at $k_cH = 3.995$, a factor 9.8 above the classical
layer) and $\mathrm{Ra} = 1674.292$ at $kH = \pi$ for $\lambda = 0$. On D3Q27
central moments with a D3Q7 scalar at $n_z = 1$, halfway walls:

\begin{center}\small
\begin{tabular}{lrrrr}
\toprule
 & $H=16$ & $H=32$ & $H=64$ & order \\
\midrule
$\lambda = 0.5$ & $+4.665\%$ & $+1.059\%$ & $+0.250\%$ & 2.14, 2.08 \\
$\lambda = 0$   & $+2.133\%$ & $+0.481\%$ & $+0.113\%$ & 2.15, 2.09 \\
\bottomrule
\end{tabular}
\end{center}

Richardson extrapolation of the last pair lands $0.019\%$ and $0.010\%$ from the
references. Nothing nonlinear is validated with this law yet: no Nusselt number
and no sidewall flow has been compared with a published result.

\subsection{Implicit Darcy drag: the mushy-zone sink}
\label{sec:darcy}

An enthalpy-porosity model holds the solid still with a Carman--Kozeny drag
$\bm{F} = -A\,\bm{u}$, $A = A_{\rm lat}\,\epsilon\,(1-f_l)^2/(f_l^3+\epsilon)$.
Built by a driver from the velocity the solver last \emph{reported}, the force
lags a step; with Guo's half shift that is the two-step recurrence
$u^{n+1} = (1-\tfrac32 A)\,u^n + \tfrac12 A\,u^{n-1}$, whose spectral radius
reaches one at $A = 1$, and the solid keeps a leakage floor of about $2\%$ of
the peak velocity. \code{DarcyGuo} instead closes the half shift with the drag,
which a linear local force allows in one line,
\begin{equation}
  \rho\bm{u} = \bm{m} + \tfrac12\big(\bm{F}_{\rm ext} - A\bm{u}\big)
  \quad\Longrightarrow\quad
  \bm{u} = \frac{\bm{m} + \bm{F}_{\rm ext}/2}{\rho + A/2},
\end{equation}
and evaluates the force at that $\bm{u}$ in the source. A uniform state then
decays by $g = (2-A)/(2+A)$ per step, $|g|<1$ for every $A>0$. Operators that
cannot be handed the velocity refuse the policy at compile time. On the reduced
channel of \code{validation/mushy\_sink} the implicit and lagged profiles agree
to $2.5\e{-14}$ wherever the lagged one converges (a steady state cannot see a
lag), every $A$ up to $10^6$ is stable on BGK and on central moments, and the
leakage falls as $1/A$: $2.79\e{-6}$ of the peak at $A = 10^4$. The stiff
sink's no-slip plane is \emph{not} on the first solid node: it lies
$0.570$/$0.736$/$0.904$ cells past the last liquid node on BGK and
$0.672$/$0.809$/$0.904$ on central moments at $\tau = 0.6/0.8/1.0$, so the
relaxation time also places the solid surface, to within $0.43$ of a cell.

\subsection{Ice on water at equilibrium}
\label{sec:ice-equilibrium}

\code{validation/ice\_equilibrium} couples the three pieces above ---
\code{EnthalpyBGK}, \code{DarcyGuo} and the density-anomaly buoyancy --- in the
configuration of Wang \emph{et al.}\ (PRFluids 6, L091501, 2021): ice grown
down from a plate at $T_t<0$ over water heated from below at $T_b$. Their
$T_b = 10\,^\circ$C comparison convects at $\mathrm{Ra}\sim10^8$ and needs days
of physical time; this case takes the limit in which the answer is exact.
With $T_b\le4\,^\circ$C every water temperature lies below the density maximum,
the layer is stably stratified, and the equilibrium is two-slab conduction,
$h/H = k_i|T_t|/(k_i|T_t|+k_wT_b)$, with water's $k_i/k_w = 3.944$ and
$(\rho c_p)_i/(\rho c_p)_w = 0.4824$. The buoyancy is sized so that the water
would sit at $\mathrm{Ra}\approx5\times10^3$ if its stratification were
unstable, and it is seeded. At every resolution the flow contributes nothing
(a frozen-fluid twin agrees to $1.2\e{-8}$ of the temperature span), the residual
velocity is below $2.5\e{-9}$ of the would-be free-fall velocity, and the seed
decays by at least $10^9$.

The isothermal front ($T_s=T_l$) has no unique discrete equilibrium: any link
whose two-slab interface temperature lies in $[-\tfrac12 s_w, \tfrac12 s_i]$,
with $s$ the per-cell temperature drop on each side, is steady, and growing ice
stops within one cell of the window's thin edge. The predicted edges are $0.906$
and $1.222$ cells; the fronts land at $0.903$ and $1.241$ at $H = 64$. The error
in cells does not refine away. A mushy band $[-d,d]$ makes $f_l$ a function of
$T$ and converges onto its own exact equilibrium, obtained in closed form from
$K(T)=\int k\,dT$: within $0.09$ cells at $H=64$, with the pointwise temperature
error falling from $1.8\%$ to $0.18\%$ and $0.094\%$ over $H = 16$--$64$. The
band is a model change, though: $d = 0.2\,^\circ$C moves the equilibrium by
$0.87$ cells here, so it must be sized against the gradient at the front.
...
\subsection{Where the height field comes from: importing a city}
\label{sec:osmimport}

The reader above takes a height field as given. Producing one from
OpenStreetMap was, until \code{tools/osm\_city.py}, something this tree could
not do: the geometry for the runs below was built by a separate project whose
script needs osmnx, geopandas, shapely, pyproj, pandas and numpy, none of which
are installed on the machine M3LB is developed on. A city could be simulated
here but not imported here, which made the first step of a new case an errand
in another repository.

The importer is stdlib only --- \code{urllib}, \code{json}, \code{math},
\code{array} --- and that is a judgement about what the job actually is rather
than an aesthetic preference. Stripped of the libraries, it is an HTTP GET
against Nominatim and Overpass, a map projection, and a polygon fill, and each
of those is under a hundred lines. \code{HeightField.hpp} already hand-rolls a
\code{.npy} reader on the same reasoning and says so in its banner.

\paragraph{The height ladder is the modelling.} A footprint's height is taken
from its \code{height} tag, failing that from \code{building:levels} times
$3.2$~m, failing that from a fallback --- either one flat number or, with
\code{-{}-fallback type}, the median storey count of the buildings in the same
query that \emph{do} carry \code{building:levels}, taken per building type.
Most of a city rests on the lower rungs: Manchester carries a \code{height} tag
on $5.7\%$ of its buildings and reaches one through \code{building:levels} on a
further $33.6\%$, leaving $60.8\%$ on the fallback. Midtown Manhattan carries
one on $95\%$, the city having imported LiDAR-derived heights --- that figure
is the reference implementation's, over $46\,327$ buildings, and not a
measurement taken here. Which rung produced each cell is written to a
companion \code{\_source.npy} and counted in the metadata, so the fraction of a
domain resting on a guess is visible rather than assumed.

Which fallback is used changes the heights and nothing else, which is worth
stating as a measurement rather than as an intention: switching Manchester from
the flat $10$~m to the type-calibrated ladder moves $14.67\%$ of cells by a
mean of $0.37$~m and a worst of $15.6$~m, raises the median built column from
$10.0$ to $12.8$~m --- houses calibrate to two storeys, apartments to three,
\code{building:part} sections to five --- and leaves the built set bit
identical, $47\,787$ cells with not one cell gained or lost. The flat default
is the worse model and is nonetheless the default here, because it is what the
existing rasters were built with and a cross-check whose two sides differ in
the model checks nothing.

Both \code{building}
and \code{building:part} must be queried: the Simple 3D Buildings schema puts a
tall structure's real geometry on \code{building:part} children while the
parent outline carries no height at all, and the reference implementation measured that querying the
parent alone leaves $20.6\%$ of Midtown's footprints by count but $54.8\%$ of
its built \emph{area} on the fallback. In Manchester, \code{building:part}
supplies $621$ of the $6\,174$ footprints.

\paragraph{It is checked against the implementation it replaces, on frozen
input.} Two rasterisers sharing no code, compared cell by cell, is the argument
\S\ref{sec:crosscolour} makes for the colour gradient, and the same is
available here because the osmnx script's Overpass response for Manchester is
still on disk. Reading that snapshot rather than re-querying is what makes the
comparison mean something: it is two rasterisers on identical input, not two
downloads taken seven weeks apart. The importer therefore parses both Overpass
response shapes --- geometry inline, and the node-reference form the cached
snapshot is written in.

\begin{center}
\begin{tabular}{lrl}
\toprule
compared against \code{mcr\_heights.npy} & identical cells & what the difference is \\
\midrule
same frozen 2026-07-30 snapshot & $160\,000/160\,000$ & nothing \\
live query today, same centre   & $159\,851/160\,000$ & seven weeks of OSM edits \\
\bottomrule
\end{tabular}
\end{center}

\noindent The first row is exact: every one of $160\,000$ cells, including all
$47\,787$ carrying a building, agrees to the bit. The second row is the same
importer against a live query at the same centre, and its $149$ differing cells
are two-sided --- $71$ gained, $8$ lost, the rest changed height --- which is
what genuine editing looks like and what a rasteriser bug does not.

\paragraph{Two defects, and only the diff could have found either.} The first
was a transpose. The raster was written at index $j\,n_x+i$ where
\code{HeightField.hpp} stores $i\,n_y+j$, which rotates the city ninety
degrees and changes no cell count, no built fraction and no mass budget. The
second was subtler and cost $39$ whole buildings, $1.4\%$ of the domain: in the
node-reference form a relation's members are bare \code{\{type, ref, role\}}
with no geometry, so resolving them in place yields nothing and every
multipolygon vanishes --- and because a multipolygon's member ways are
\emph{untagged}, the tags living on the relation, the ordinary way loop does
not pick them up either. Both were silent. The first showed as $98.36\%$
agreement with every difference one-sided, which is the signature of missing
footprints rather than a boundary rule; the second was localised by asking
which raw OSM feature covered the largest missing blobs, and the answer came
back as untagged ways, one of them carrying the note
\emph{``tags on relation now''}.

Neither is reachable by a test the importer could run against itself, and the
Manchester comparison needs a $4$~MB snapshot outside this repository, so what
\code{ctest} carries instead is the cheapest guard against the class: the
importer writes a deliberately non-square fixture city and \code{height\_field}
reads it back and checks the writer's own recorded answers. Transposing that
fixture by hand fails exactly one check, while the built fraction, the tallest
column and the solid count --- $27$ cells either way --- all still pass, which
is the demonstration that every total is blind to it.

\paragraph{A place name is not a coordinate.} Geocoding
\emph{``Manchester city centre, UK''} today returns a point $1\,459$~m from
the one the same string returned in July, because Nominatim matches
\emph{City Centre} inside a postal address and both hits are offices rather
than districts. On a $2$~km domain that is $73\%$ of a side. So
\code{-{}-lat/-{}-lon} exists, the resolved coordinate is written into the
metadata, and the provenance of a raster is that coordinate and the cached
response --- never the place string that produced them.

\paragraph{What it does not do.} There is no terrain: the ground is flat at
$z=0$, which is what the solver assumes. There are no roof shapes and no
overhangs --- a column is solid from the ground to the tallest footprint
covering it, and nothing can be solid above empty air. Relations are assembled
by stitching member ways into rings, which handles a city centre's
multipolygons but is not a complete OSM multipolygon implementation; leftovers
that will not close are counted and dropped rather than guessed at.
...
\subsection{One overlap between the two code bases is enforced, not checked by
hand}
\label{sec:crosscolour}

\S\ref{sec:cuda} states that \code{src/} and \code{GPU/} agree where they
overlap and that a disagreement is a bug in one of them. Until
\code{tests/cross\_colour} nothing enforced it: the rule was checked by running
a case on each and comparing printed diagnostics, which is slow, noisy, and
exercises only whatever states that case happens to visit.

The test drives \emph{both} colour-gradient operators directly --- no simulation,
milliseconds --- over 320 identical randomised states at
$\gamma=1,10,100,1000$ under both rest-term readings, and diffs all 27
populations. It reaches the branches a droplet never visits:
$|\nabla\phi|=0$, $A=0$, $\rho_{\rm ref}$ set and unset, $\omega_{\rm bulk}$
away from $\omega$, a body force, and an input deliberately off equilibrium. The
worst difference is \textbf{0 exactly, at both precisions} --- sharper than it
sounds, since two independently written implementations also \emph{round}
identically.

Four things about it are deliberate. Both trees put their lattice and their
operator in \code{namespace lbm}, so one translation unit including both headers
collides on \code{D3Q27} and half a dozen other names; the two objects are
therefore joined by \code{extern "C"}. It lives in the \emph{parent} and reaches
into \code{GPU/include} rather than the reverse, because the parent already links
Kokkos --- which \code{ColourGradient.hpp} needs for its node-field Views --- and
\code{GPU/} deliberately does not: making \code{GPU/}'s CMake acquire Kokkos so
that it could be cross-checked would spend exactly the independence that makes
the comparison mean anything. The two precision defaults are \emph{opposite}
(\code{GPU/}'s \code{Real} is \code{float} unless \code{LBM\_DOUBLE} is
defined, the parent's is \code{double} unless \code{LBM\_SINGLE\_PRECISION}
is), and mismatching them still compiles and reports a disagreement of about
$10^{-7}$ --- FP32 epsilon, indistinguishable at a glance from a physics bug, and
it cost a real detour while this was being written; the CMake sets
\code{LBM\_DOUBLE} on the \code{GPU} object exactly when the parent is FP64.
And the velocity orderings are \emph{checked rather than assumed}: both are
D3Q27 and both number their velocities identically today, but the harness matches
slot to slot by integer velocity first and reports what it found, because
comparing two differently ordered sets would produce a large, structured and
entirely spurious disagreement --- the kind that reads as physics.

\textbf{It has been shown to fail.} A test that passes and has never been
demonstrated to fail is a test of nothing, and this document records that lesson
elsewhere under \code{GPU/}'s gather/scatter bug. Perturbing one coefficient in
one of \code{cg\_source\_high}'s seventeen machine-generated slots by $10^{-7}$
\emph{relative} --- $1/3$ written as $1/3.0000001$, far smaller than any
transcription error a human makes --- turns seven of the nine checks red at a
worst population difference of $4.1\times10^{-11}$. The tolerance therefore sits
about four orders below the smallest plausible real defect.

The target is skipped, with a message, when \code{GPU/} is absent; that this is
one repository today is an assumption confined to that one \code{CMakeLists}.
The pattern generalises to any other operator the two trees share.
...
\item \textbf{An on-node regularised fluid wall under a non-uniform body force
  diverged over $\sim10^5$ steps, and the mechanism is not diagnosed}
  (\S\ref{sec:ice-equilibrium}). With the density-anomaly force the first
  version of \code{ice\_equilibrium} went non-finite at $1.7\times10^5$ and
  $4.2\times10^5$ steps, the growth beside the water wall where no drag acts;
  the halfway pair ran $2.7\times10^6$ steps clean. The on-node Rayleigh--B\'enard
  numbers are growth rates over $0.75$ diffusive times and cannot see it.
\item \textbf{An isothermal enthalpy front is first order at equilibrium.}
  Its steady position is any link in a window about a cell wide, and which one it
  reaches depends on the direction it came from (\S\ref{sec:ice-equilibrium}).
  A mushy band removes the ambiguity at the cost of a model error set by its
  width.
