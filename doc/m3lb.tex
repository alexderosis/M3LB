...
\paragraph{What has not been measured.} \S\ref{sec:ehdconv}'s free-slip variant
still runs the halfway fluid mirror against an on-node scalar wall, so the gap
recorded there has not been re-measured with both families on one plane. Writing
the wall does not by itself close that gap. \code{GPU/} carries the same
condition (\code{specular.cuh}), asserted the same way in
\code{test/host\_physics.cpp}, at $1.6\times10^{-15}$ and $1.4\times10^{-15}$ in
FP64 and $8\times10^{-7}$ in FP32.
...
\item \textbf{There are now two specular fluid walls, on different planes, and
  the free-slip electroconvection gap has not been re-measured with the new
  one.} \S\ref{sec:specular}'s is a ghost cell, half a cell outside the last
  fluid node; \S\ref{sec:specnode}'s is on-node, and pairs with the on-node
  scalar wall of \S\ref{sec:scalarspec}. Each is exact against the box it
  mirrors --- $5.5\times10^{-14}$, $1.9\times10^{-15}$ and $5.3\times10^{-15}$
  respectively --- but exact about different planes, and the same box closed by
  the wrong one is out by $16\,\%$ of the field. \S\ref{sec:ehdconv}'s free-slip
  variant still runs the halfway fluid mirror against an on-node scalar wall, so
  that geometry's gap must still not be read as a measurement of the scalar wall
  alone. The wall now exists; the measurement that would close the question does
  not. Axis-aligned faces only in both, and the ghost version additionally
  refuses edges.
