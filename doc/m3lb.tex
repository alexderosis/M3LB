...
\paragraph{The side walls, and a substitution.} Their Eqs.~(14)--(16) put
zero-gradient $\varphi$ and $q$ and $\uu\!\cdot\!\nn=0$ on the lateral sides ---
free-slip, neither periodic nor no-slip. \textbf{This tree has no free-slip
wall}, and adding specular reflection to Esoteric Pull is not a small change:
bounce-back is cheap there only because opposite directions occupy adjacent
slots (\S\ref{sec:esopull}), and a mirror about a wall normal is a different
permutation. So the case uses the exact equivalence instead --- a free-slip box
of width $L_x$ is the mirror-symmetric half of a \emph{periodic} box of width
$2L_x$. On the mirror planes $u_x$ is odd and $\varphi$ and $q$ are even, which
is Eqs.~(14)--(16) exactly, and the doubled box's smallest admissible wavenumber
$\pi/L_x$ is precisely the most unstable mode. It costs twice the cells and
imposes nothing --- but it \emph{admits} laterally antisymmetric modes that
free-slip forbids, so the wavelength is measured rather than assumed: every run
reports the dominant lateral wavenumber of $u_y$ at mid-depth and its share of
...
grid barely matters ($3.7522\to3.7597$ from $n_y=81$ to $163$, $+0.20\%$). At
...
\item \textbf{There is no free-slip wall.} The lateral boundary of the
  electroconvection case (\S\ref{sec:ehdconv}) is $\uu\!\cdot\!\nn=0$ with
  zero-gradient scalars, and that condition is not implemented: specular
  reflection is a different permutation from bounce-back, whose cheapness under
  Esoteric Pull comes from opposite directions occupying adjacent slots. The
  case works around it with a doubled periodic box, which is exactly equivalent
  for the mirror-symmetric state and is verified rather than assumed --- but a
  geometry that genuinely needs one free-slip wall, without a mirror partner,
  cannot be built.
