//==============================================================================
//  Shercliff's problem: fully developed MHD flow in a rectangular duct with
//  INSULATING walls, against the analytic solution.
//
//  Reference: J. A. Shercliff, Proc. Camb. Phil. Soc. 49, 136 (1953). The
//  companion case with conducting walls is Hunt, J. Fluid Mech. 21, 577 (1965)
//  and is NOT here -- see the note at the end.
//
...
//  here" -- it has Dirichlet and an unvalidated streamwise zero-gradient and
//  nothing else. Hunt therefore needs that condition written first; this file
...
int main(int argc, char** argv) {
  ...
  int status = 0;
  {
    ...
    std::printf("Shercliff duct -- insulating walls, against the analytic series   %s\n",
                ExecSpace::name());
    ...
    for (const Index ny : Nys) {
      ...
      for (const double Ha : Has) {
        ...
        for (Index y = 1; y < ny - 1; ++y) {
          double uu, bb;
          shercliff((double(y) - L) / L, 0.0, Ha, l, K, uu, bb);
          up = std::max(up, uu);
        }
        ...
        auto exact_err = [&]() {
          ...
          for (Index z = 0; z < nz; ++z)
            for (Index y = 0; y < ny; ++y) {
              double ue, be;
              shercliff((double(y) - L) / L, (double(z) - 0.5 * double(nz - 1)) / L,
                        Ha, l, K, ue, be);
              ue *= Uref;
        ...
        std::printf("  %5d %6d %8.1f %9.5f %7.2f %7.2f %9.5f %11.4e %s %10zu %9.1e%s%s\n",
                    int(ny), int(nz), Ha, double(B0),
                    Ha > 0 ? L / Ha : 999.0, Ha > 0 ? L / std::sqrt(Ha) : 999.0,
                    umx, eu, ebs, taken, res,
                    (taken >= cap) ? "  CAPPED" : "",
    ...
    std::printf("  Hunt's conducting-wall case needs db_x/dn = 0, a Neumann magnetic\n"
                "  condition MagneticSolver does not have. Not attempted here.\n");
  }
  Kokkos::finalize();
  return status;
}
