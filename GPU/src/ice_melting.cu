...
struct MeltInit {
  ...
  LBM_HD Real operator()(int x, int y, int z) const {
    if (x <= xice) return m.enthalpy_of(Real(0), Real(0));
...
int main(int argc, char** argv) {
  ...
  const int H = o.H, W = int(std::lround(o.aspect * H)), nx = W + 2, ny = H + 2, nz = o.nz;
  ...
  std::printf("  box %d x %d x %d, ice %d columns  kappa %.3e nu %.3e  tau_f %.4f tau_T %.4f  "
              "%s scalar  %s, %s\n\n", W, H, nz, xice, kappa, nu, 3 * nu + 0.5, 4 * kappa + 0.5,
              o.reg ? "regularised" : "BGK", dev.name.c_str(), sizeof(Real) == 8 ? "FP64" : "FP32");
  ...
  for (int z = 0; z < nz; ++z)
    for (int y = 0; y < ny; ++y)
      for (int x = 0; x < nx; ++x) {
        ...
        if (x == 0)            { sf[c] = ScalarDirichlet; sw[c] = H_ice; }          // E = H at f = 0
  ...
  sc.initialise_with(MeltInit{xice, Real(o.Tinf), Real(o.noise), pc});
  ...
  auto measure = [&](bool with_flow) {
    ...
    for (int z = 0; z < nz; ++z)
      for (int y = 1; y <= H; ++y) {
        ...
        th[std::size_t(z) * H + std::size_t(y - 1)] = t;
        vol += t;
        for (int x = int(t) + 21; x <= W; ++x, ++nb)
  ...
  auto bl_velocity = [&]() {                 // 8 cells beside the ice, middle half of the height
    ...
    for (int z = 0; z < nz; ++z)
      for (int y = H / 4; y <= 3 * H / 4; ++y) {
        const int x0 = int(std::lround(th[std::size_t(z) * H + std::size_t(y - 1)])) + 1;
  ...
  while (step < cap) {
    for (long k = 0; k < o.probe; ++k) { sc.compute_field(); melt_pass(mp); fl.step(); sc.step(); }
    step += o.probe;
  ...
  if (finite) {
    ...
    if (!o.out.empty()) {
      std::FILE* fp = std::fopen((o.out + "_thickness.txt").c_str(), "w");
      if (fp) {
        std::fprintf(fp, "# ice thickness h(y, z) in cells; rows y = 1..%d, columns z = 0..%d\n", H, nz - 1);
        for (int y = 0; y < H; ++y) {
          for (int z = 0; z < nz; ++z) std::fprintf(fp, " %.5f", th[std::size_t(z) * H + std::size_t(y)]);
          std::fprintf(fp, "\n");
        }
        std::fclose(fp);
        std::printf("  wrote %s_thickness.txt\n", o.out.c_str());
      }
    }
  }
  return finite ? 0 : 1;
