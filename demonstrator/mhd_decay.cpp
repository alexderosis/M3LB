...
//
//  -op bgk NEEDS A COMFORTABLE tau AND WILL NOT GET ONE AT A SMALL N. Because
//  nu = u0 (2R) / Re and R scales with the box, the relaxation time here is set
//  by the RESOLUTION and not chosen independently: tau = 0.543 at the default
//  N = 321 and only 0.510 at N = 97. Measured, BGK goes non-finite inside one
//  turnover at N = 97 while central moments decay cleanly at the same point,
//  which is the same ordering validation/square_cylinder.cpp measured at
//  tau = 0.512. Central moments are the default for that reason; -op bgk is for
//  comparison at a resolution that can afford it.
//
...
struct Opts {
  ...
  std::string vtkdir, bindir, walls = "insul", shape = "disc";
};
...
static int run(const Opts& o) {
  ...
  std::vector<double> hux(std::size_t(N) * N, 0.0), huy(hux), hbx(hux), hby(hux);
  {
    ...
    for (int kx = -o.k0 - o.kw; kx <= o.k0 + o.kw; ++kx)
      for (int ky = 0; ky <= o.k0 + o.kw; ++ky) {
  ...
  mag.set_geometry([&](Index x, Index y, Index) { return solid(x, y); });
  ...
  std::printf("  u_rms %.4f (Ma %.4f)   k0 %d +- %d   E_kin/E_mag %.2f   "
              "walls %s   seed %llu\n", o.u0, o.u0 / 0.5773502692, o.k0, o.kw,
              o.alf, cond ? "perfectly conducting (Neumann, all faces)"
  ...
  std::printf("  %8s %8s %11s %11s %9s %11s %11s %7s %7s %8s %11s %11s %9s %9s\n",
  ...
  for (std::size_t t = 0; t <= o.steps; ++t) {
    if (t % o.probe == 0) {
      ...
      if (o.binevery && (t % o.binevery == 0)) {
        auto emit = [&](const char* tag, bool current) {
          char nm[512];
          std::snprintf(nm, sizeof nm, "%s/decay_%s_%s%04zu.bin", o.bindir.c_str(),
                        o.walls.c_str(), tag, bframe);
      ...
      if (o.vtkevery && (t % o.vtkevery == 0)) {
        ...
        std::snprintf(nm, sizeof nm, "%s/decay_%s_%04zu.vtk", o.vtkdir.c_str(),
                      o.walls.c_str(), vframe++);
        write_vtk(nm, N, d, a1, a2, a3, a4, solid);
      }
    }
    if (t < o.steps) { mag.compute_field(); fl.step(true); mag.step(true); }
  }
